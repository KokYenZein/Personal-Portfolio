            ...
            <ol className={classNames(styles['education'], 'border-l border-gray-200 dark:border-gray-500')}>
              {educations.map((education, index) => (
                <li key={index} className={styles['education-item']}>
                  ...
                  <h5 className="mb-1 block text-base font-semibold text-gray-700 dark:text-gray-200">
                    {education.grade}
                  </h5>
                  <p dangerouslySetInnerHTML={{ __html: education.description.replace(/\n/g, '<br />') }} />
                  <p className="mb-4 text-base font-normal text-gray-700 dark:text-gray-200">
                    <strong>Relevant Courses:</strong> {education.relevantCourses.join(', ')}.
                  </p>
                </li>
              ))}
            </ol>
